...
namespace ApplesGame
{
	...
	void HandleGameStateMainMenuWindowEvent(GameStateMainMenuData& data, Game& game, const sf::Event& event)
	{
		...
		if (event.type == sf::Event::KeyPressed)
		{
			...
			else if (event.key.code == sf::Keyboard::Enter)
			{
				...
				{
					game.options = (UserGameSettings)((std::uint8_t)game.options ^ (std::uint8_t)UserGameSettings::InfiniteApples);
				}
				else if (data.menu.selectedItem == &data.optionsWithAccelerationItem)
				{
					game.options = (UserGameSettings)((std::uint8_t)game.options ^ (std::uint8_t)UserGameSettings::WithAcceleration);
				}
	...
	{
		bool isInfiniteApples = ((std::uint8_t)game.options & (std::uint8_t)UserGameSettings::InfiniteApples) != (std::uint8_t)UserGameSettings::Empty;
		data.optionsInfiniteApplesItem.text.setString("Infinite Apples: " + std::string(isInfiniteApples ? "On" : "Off"));

		bool isWithAcceleration = ((std::uint8_t)game.options & (std::uint8_t)UserGameSettings::WithAcceleration) != (std::uint8_t)UserGameSettings::Empty;
